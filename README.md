...
Both the `web-app` and `database` containers are connected to this network. This allows the web application to communicate with the MySQL database using the hostname `database`.

### Persistent Volume

A named Docker volume called `mysql-data` is used to store the MySQL database data.

The data is preserved even when the containers are stopped and restarted. The volume is only removed when `./remove-app.sh` is executed.
